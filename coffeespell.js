...
async function getItemList(section, url) {
    ...
    for(let article of $('li.-qHwcFXhj0')) {
        ...
        item.set('detail', $(article).find('p.vChbm1yu9U').text() || '')
...
async function init() {
    ...
    let now = new Date();
    obj.set("today", now.getFullYear + "." + (now.getMonth < 9 ? "0" : "") + now.getMonth + 1 + "." + now.getDate)
    ...
    for(let page of 생두pages) {
        ...
        itemList.push(...await getItemList('생두', page))
    }
    console.log(itemList)
    // let 원두list = await getPageList('https://smartstore.naver.com/coffeespell/category/ff37e49dadcc4f17b1148766d247b2bb')
}
